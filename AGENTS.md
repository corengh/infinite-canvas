...
- 每次写完代码，按根目录 `AGENTS.md`、对应任务卡和测试策略执行与改动风险相称的格式、类型、测试或构建验证；用户明确要求跳过时除外。
- 不要改无关文件，不要顺手重构。
...
- 画布项目以平台服务端文档和 op-log 为持久化真相，浏览器 IndexedDB 只保存离线快照与待同步操作；
  资产与提示词同样以平台服务端为真相源，签名 URL 只作短期读取凭证。
- 用户站不保存供应商 API Key 或自定义 baseUrl，所有模型生成请求统一经过平台后端计价与调度。
