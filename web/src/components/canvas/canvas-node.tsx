...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <NodeContent
                        ...
                        onRetryBatchImage={onRetryBatchImage ? (imageId) => onRetryBatchImage(data, imageId) : undefined}
            ...
            {/* [PLATFORM] 接缝 #18：Audio 节点保留上传与播放能力，MVP 隐藏生成面板（D-17）。 */}
            {showPanel && !isGroup && data.type !== CanvasNodeType.Audio && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-[600px] -translate-x-1/2 pt-4">{renderPanel(data)}</div> : null}
        </div>
    );
});
...
function NodeContent(props: NodeContentRendererProps) {
    ...
    if (props.node.metadata?.status === "error") return <ErrorContent node={props.node} theme={props.theme} onRetry={props.node.type === CanvasNodeType.Audio ? undefined : props.onRetry} />;
...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    ...
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            ...
            {node.metadata?.generationStatus && node.metadata.generationStatus !== "succeeded" ? (
                <div className="text-xs font-medium" style={{ color: theme.node.activeStroke }}>
                    积分已退还
                </div>
            ) : null}
            {onRetry ? (
...
function ImageContent({
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded} onToggleBatch={onToggleBatch}>
            ...
            {primaryImage?.status === "error" ? <BatchImageFailureActions placement="left" onRetry={onRetryBatchImage ? () => onRetryBatchImage(primaryImage.id) : undefined} onDelete={() => onDeleteBatchImage?.(primaryImage.id)} /> : null}
