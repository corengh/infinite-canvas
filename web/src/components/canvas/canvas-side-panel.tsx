...
import { Check, ChevronRight, Download, Eye, FileText, Image as ImageIcon, ListChecks, Music2, Plus, Search, Square, Trash2, Type, Video } from "lucide-react";
...
function buildInsertPayload(asset: Asset): InsertAssetPayload {
    ...
    const assetId = asset.data.storageKey === asset.id ? asset.id : undefined;
    if (asset.kind === "video") return { kind: "video", assetId, url: asset.data.url, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height };
    if (asset.kind === "image") return { kind: "image", assetId, dataUrl: asset.data.dataUrl, storageKey: asset.data.storageKey, title: asset.title };
    throw new Error("该资产类型暂不支持插入画布");
}
...
const CanvasAssetsTab = memo(function CanvasAssetsTab({ onInsert, theme }: { onInsert: (payload: InsertAssetPayload) => void; theme: CanvasTheme }) {
    ...
    useEffect(() => {
        void loadAssets({ limit: 100 }).catch((error) => message.error(error instanceof Error ? error.message : t("fe9.assets.loadFailed")));
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                {groups.length ? (
                                                        ...
                                                        asset.ownerId === userId
                                                            ? async () => {
                                                                  try {
                                                                      ...
                                                                      message.error(error instanceof Error ? error.message : t("canvas.sidePanel.assetRemoveFailed"));
                                                                  }
                                                              }
                                                            : undefined
                                                    }
                    ...
                    <>{loading ? <Spin className="mt-16 block" size="small" /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t("canvas.sidePanel.noAssets")} className="pt-16" />}</>
                )}
                {nextCursor ? (
                    <button
                        type="button"
                        disabled={loading}
                        onClick={() => void loadAssets(undefined, true).catch((error) => message.error(error instanceof Error ? error.message : t("fe9.assets.loadFailed")))}
                        className="mx-auto mt-3 block rounded-md px-3 py-1.5 text-xs transition hover:bg-black/5 disabled:opacity-50 dark:hover:bg-white/10"
                    >
...
function AssetCover({ asset }: { asset: Asset }) {
    ...
    if (asset.kind === "image") return <img src={asset.coverUrl || asset.data.dataUrl} alt="" className="size-full object-cover transition duration-300 group-hover:scale-[1.04]" />;
    return <div className="grid size-full place-items-center text-xs opacity-60">{asset.title}</div>;
}
...
const CanvasPromptsTab = memo(function CanvasPromptsTab({ onInsert, theme }: { onInsert: (payload: InsertAssetPayload) => void; theme: CanvasTheme }) {
    const { t } = useTranslation();
    const [keyword, setKeyword] = useState("");
    const query = useQuery({ queryKey: ["side-panel-prompts", keyword], queryFn: () => promptsApi.list({ search: keyword.trim() || undefined, limit: 100 }), staleTime: 30_000 });
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                ...
                {query.isError ? (
                    <button type="button" onClick={() => void query.refetch()} className="w-full py-8 text-xs text-red-500">
                        {t("canvas.sidePanel.loadFailedRetry")}
                    </button>
                ) : null}
                <div className="space-y-1.5">
                    {query.data?.items.map((item) => (
                        <div key={item.id} className="group flex items-center gap-2.5 rounded-lg px-2 py-2 transition hover:bg-black/5 dark:hover:bg-white/5">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-0.5 truncate text-xs opacity-50">{item.content}</div>
                            </div>
                            <button
                                type="button"
                                onClick={() => onInsert({ kind: "text", content: item.content, title: item.title })}
                                className="grid size-7 place-items-center rounded-md opacity-60 transition hover:bg-black/10 hover:opacity-100 dark:hover:bg-white/10"
                                aria-label={t("canvas.sidePanel.inserted")}
                ...
                {!query.isLoading && !query.data?.items.length ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t("canvas.sidePanel.noPrompts")} className="pt-12" /> : null}
            </div>
        </div>
    );
});
