...
+ [调整] 删除 WebDAV 同步，隐藏独立生成工作台、本地 Canvas Agent 与 Audio 生成入口，并补齐上游合并和工程守卫机制。
+ [新增] 资产库与提示词库改用服务端存储和团队共享，平台配置支持跨设备偏好，首页应用入口改为注册表动态渲染。
+ [调整] 删除用户侧模型密钥、渠道配置与画布配置节点，并为旧画布自动清理遗留配置节点和悬空连线。
+ [新增] 画布新增单人编辑锁、只读提示与接管流程，并在失锁时保留未提交修改供导出。
+ [新增] 画布项目改用服务端持久化，支持增量同步、断网续传、冲突恢复与未保存修改导出。
+ [修复] 画布刷新、版本冲突、批量删除及跨画布切换时保持服务端、离线队列与当前界面状态一致。
+ [调整] 图像、视频与文本生成改走平台后端任务，画布支持真实进度、排队位置、取消退款、多版本结果和刷新恢复。
+ [修复] 统一生成预估与提交参数，并修复超时轮询、批量任务刷新恢复及取消期间重复提交。
+ [新增] 画布生成新增服务端模型选择、动态参数、实时积分预估和统一确认，并支持纯提示词生成视频。
+ [新增] 平台新增充值档位、支付宝与微信扫码支付、订单状态轮询及充值记录页面。
+ [修复] 充值订单隔离浏览器历史中的旧账号数据，补齐金额异常状态，并让重复下单键真正回放原订单。
+ [新增] 平台新增钱包、积分流水、团队成员与组管理、划拨回收、补拨审批、闲置排行及团队设置页面。
+ [修复] 钱包与团队数据按登录身份隔离缓存，并在资金、成员和组变更后同步刷新关联视图。
+ [新增] 平台账号支持手机号或成员账号登录、主账号注册、短信找回密码、账号与设备管理及会话过期原地重登。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [调整] 新增平台运行时配置基建，并移除远程插件下载、执行及管理入口。
