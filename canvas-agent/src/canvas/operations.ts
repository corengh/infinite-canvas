        ...
        return applyOps([
            {
                ...
                title: data.title,
                position: { x: data.x ?? nextCanvasX(state), y: data.y ?? 0 },
                width: data.width,
                height: data.height,
...
/** 创建直接承载生成参数和结果的目标节点，平台画布不再引入 Config 中间节点。 */
function generationTargetNodeOp(id: string, input: Record<string, unknown>, mode: "text" | "image" | "video" | "audio", prompt: string, x: number, y: number) {
    return {
        type: "add_node",
        id,
        nodeType: mode,
...
/** 创建提示词、真实目标节点和引用连线，生成结果直接写回目标节点。 */
